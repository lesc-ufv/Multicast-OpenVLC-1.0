...
def mc_recv(fromnicip, mcgrpip="224.1.1.5", mcport=50001):
    ...
    datagram, _ = receiver.recvfrom(buffer)
    print("Recebendo datagrama 1")
    cont = 1
    parts = []
    try:
        while(datagram):
            payload = pickle.loads(datagram)
        ...
        ordemRecebida = []
        for j in parts:
            ordemRecebida.append(j[1])

        cont = 1
        naoPresentes = []
        for i in parts:
            if(cont != i[1]):
                #print("Erro! Diferente em cont = ", cont, "part = ",i[1])
                try:
                    index = ordemRecebida.index(cont)
                except:
                    #print("Erro! O datagram de numero ", cont, " nao está presente!")
                    naoPresentes.append(cont)
                if(index and parts[index][1]==cont):
                    f.write(parts[index][0])
                    print("Erro de ordem de datagram corrigido!")
            else:
                f.write(i[0])
            cont+=1
        if(len(naoPresentes)>0): print("Datagrams não presentes:",naoPresentes)
        f.close()
