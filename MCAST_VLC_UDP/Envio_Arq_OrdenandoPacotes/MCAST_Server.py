...
def MCAST_Server(hostip, mcgrpip="224.1.1.5", mcport=50001):
    ...
    while(archive != "stop"):
            ...
            sender.sendto(archive.encode(), mcgrp)
            ...
            datagram = f.read(buffer)
            cont = 1
            while(datagram):
                payload = pickle.dumps([datagram,cont])
                print("Tamanho:",sys.getsizeof(payload))
                if(sender.sendto(payload, mcgrp)):
                    print("Enviando datagrama ",cont,"...")
                    cont +=1
                    sleep(0.0625)
                    datagram = f.read(buffer)
            ...
            sleep(2)
            sender.sendto(digest.encode(),mcgrp)
            f.close()
